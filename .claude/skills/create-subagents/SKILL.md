...
<objective>
Subagents are specialized Claude instances that run in isolated contexts with focused roles. This skill teaches you how to create effective subagents, write strong system prompts, configure tool access, and orchestrate multi-agent workflows using the Task tool.
...
- Do not constrain subagent tooling without explicit instructions from the human partner, offer this option if it wasn't mentioned at all
</field>
...
Review the `../authoring-skills/persuasion-principles.md` document for proper nuance when writing instructions.
